      ...
      <div>
        ...
        >
          ...
          {senas.map((sena, index) => (
              <CarouselItem key={`sena-${sena.id}`} className="pl-3 md:pl-8 basis-1/3 lg:basis-1/4 xl:basis-1/5 transform-gpu will-change-transform">
                <div className="h-full">
                  <Card className="bg-transparent border-none overflow-hidden group cursor-pointer h-full">
                    <CardContent className="flex flex-col items-center justify-start p-0 relative h-full">
                      <Link to={`/sena/${sena.id}`} onClick={() => sessionStorage.setItem('returnScroll', 'senas-section')} className="relative w-full aspect-[2/3] overflow-hidden rounded border border-white/10 group-hover:border-[#E8832A]/50 transition-colors duration-300 block shadow-lg transform-gpu">
                        <img
                          src={sena.image}
                          alt={sena.name}
                          loading="lazy"
