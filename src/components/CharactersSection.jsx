      ...
      <div>
        ...
        >
          ...
          {characters.map((char, index) => (
              <CarouselItem key={`char-${char.id}`} className="pl-3 md:pl-8 basis-1/3 lg:basis-1/4 xl:basis-1/5 transform-gpu will-change-transform">
                <div className="h-full">
                  <Card className="bg-transparent border-none overflow-hidden group cursor-pointer h-full">
                    <CardContent className="flex flex-col items-center justify-start p-0 relative h-full">
                      <Link to={`/character/${char.id}`} onClick={() => sessionStorage.setItem('returnScroll', 'characters-section')} className="relative w-full aspect-[2/3] overflow-hidden rounded border border-white/10 group-hover:border-[#D4AF37]/50 transition-colors duration-300 block shadow-lg transform-gpu">
                        <img
                          src={char.image}
                          alt={char.name}
                          loading="lazy"
