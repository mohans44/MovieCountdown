      ...
      <div>
        ...
        >
          ...
          {kandas.map((kanda, index) => (
              <CarouselItem key={`kanda-${kanda.id}`} className="pl-3 md:pl-8 basis-1/3 lg:basis-1/4 xl:basis-1/5">
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  <Card className="bg-transparent border-none overflow-hidden group cursor-pointer h-full">
                    <CardContent className="flex flex-col items-center justify-start p-0 relative h-full">
                      <Link to={`/kanda/${kanda.id}`} onClick={() => sessionStorage.setItem('returnScroll', 'kandas-section')} className="relative w-full aspect-[2/3] overflow-hidden rounded border border-white/10 group-hover:border-[#E8832A]/50 transition-colors duration-700 shadow-[0_0_20px_rgba(0,0,0,0.6)] block">
                        <img
                          src={kanda.image}
                          alt={kanda.name}
                          loading="lazy"
