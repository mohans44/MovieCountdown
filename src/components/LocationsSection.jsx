      ...
      <div>
        ...
        >
          ...
          {locations.map((location, index) => (
              <CarouselItem key={`location-${location.id}`} className="pl-3 md:pl-8 basis-1/3 lg:basis-1/4 xl:basis-1/5">
                <motion.div
                  initial={{ opacity: 0, y: 30 }}
                  whileInView={{ opacity: 1, y: 0 }}
                ...
                >
                  <Card className="bg-transparent border-none overflow-hidden group cursor-pointer h-full">
                    <CardContent className="flex flex-col items-center justify-start p-0 relative h-full">
                      <Link to={`/locations/${location.id}`} onClick={() => sessionStorage.setItem('returnScroll', 'locations-section')} className="relative w-full aspect-[2/3] overflow-hidden rounded border border-white/10 group-hover:border-[#D4AF37]/50 transition-colors duration-700 shadow-[0_0_20px_rgba(0,0,0,0.6)] block">
                        <img
                          src={location.image}
                          alt={location.name}
                          loading="lazy"
